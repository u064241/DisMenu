...
ERRLOG = Path(TEMP) / "DisMenu_Errors.log"
OKCNT = 0
FAILCNT = 0
# ===== Output e colori =====
...
VERBOSE: bool = False
VERBOSE_FILE: Path = Path(TEMP) / "DisMenu_Verbose.log"
...
# Config persistente: %APPDATA%\DisMenu\settings.json (fallback nella cartella dello script)
APPDATA = os.environ.get("APPDATA")
if APPDATA:
    CONFIG_DIR = Path(APPDATA) / "DisMenu"
else:
...
def _find_readme() -> Optional[Path]:
    """Trova README_dismenu.md accanto all'eseguibile o allo script."""
    for base in (_app_dir(), Path(__file__).resolve().parent):
        cand = base / "README_dismenu.md"
        if cand.exists():
            return cand
    return None
...
def menu_help() -> None:
    ...
    if readme:
        ...
        try:
            ...
            opened = False
            try:
                ret = ctypes.windll.shell32.ShellExecuteW(None, "open", str(readme), None, None, 1)
                opened = int(ret) > 32
            except Exception:
                opened = False
            if not opened:
            ...
            if not opened:
                try:
                    opened = bool(webbrowser.open(readme.as_uri()))
                except Exception:
                    opened = False
            if not opened:
                print("\n--- Quick guide ---\n" + HELP_TEXT)
...
try:
    from prompt_toolkit import prompt as ptk_prompt  # type: ignore
    from prompt_toolkit.completion import PathCompleter, FuzzyCompleter  # type: ignore
...
def input_path(prompt: str) -> str:
    if HAVE_PTK and sys.stdout.isatty():
        try:
            base_completer = PathCompleter(expanduser=True)
            completer = FuzzyCompleter(base_completer)
            # Keybindings: TAB avvia/cicla completamento; SHIFT+TAB/Up/Down navigano
            kb = KeyBindings()
            ...
            def _toolbar():
                return "  TAB: completa  •  ↑/↓ o SHIFT+TAB: naviga  •  Invio: conferma  •  Ctrl+C: annulla"

            return ptk_prompt(
                prompt,
                completer=completer,  # type: ignore
                complete_while_typing=True,
                key_bindings=kb,  # type: ignore
...
def main() -> None:
    ...
    os.system("title DisMenu - DISM toolkit (Python)")
    print("DisMenu - DISM toolkit (Python)")
    # Pulizia log
    try:
        if ERRLOG.exists():
            ERRLOG.unlink()
